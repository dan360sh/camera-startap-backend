...
export class WebSocketService {
    ...
    clearUser (codeWs: string) {
        if(codeWs) {
            const index = this.listUsers.findIndex(user => user.ws === codeWs);
            const index2 = this.users.findIndex(user => user.ws.codeWs === codeWs);
            if (index2 !== -1) {
    ...
    //Переделать на отправку одного юзера
    reloadUserList() {
        for(let user of this.users) {
            user.ws.send('user_list', this.listUsers);  
        }
    }
